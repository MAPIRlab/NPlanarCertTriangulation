                        ...
                        const PerturbRotation& perturbRotation)
{
        ...
        PCRes res = PCRes(options.n_rows * options.n_cols);
        ...
        /* 
        we generate the plane 
        with distance d_plane from the world frame 
        and with normal [0, 0, 1]^T. 
        The plane has dimension 2N X 2M 
        We create a grid with n_rows x n_cols evenly spaced points
        */
        
        double size_row = 2 * options.height / options.n_rows; 
        double size_col = 2 * options.width / options.n_cols; 
        int N_points = 0; 
        
        for (int i_row = 0; i_row < options.n_rows; i_row++)
        {
        
                double x_row = i_row * size_row - options.height;  // center 
                
                
                for (int j_col = 0; j_col < options.n_cols; j_col++)
                {
                    double y_col = j_col * size_col - options.width;               
                   
                    // save point 
                    res.points_3D.col(N_points) << x_row, y_col, options.d_plane; 
                    N_points++;
                }
...
int generateLinGraph(const int M,
